...
const mockUseChatMessagesQuery = vi.fn();
const mockUseSendChatMessageMutation = vi.fn();
...
vi.mock("../../api/chat", () => ({
  ...
  useChatMessagesQuery: (...args: unknown[]) => mockUseChatMessagesQuery(...args),
  useSendChatMessageMutation: (...args: unknown[]) => mockUseSendChatMessageMutation(...args)
}));
...
describe("ChatViewPage", () => {
  it("loads chat history and renders message types", () => {
    setupBaseMocks();
    mockUseChatMessagesQuery.mockReturnValue({
      data: {
        items: [
          {
            id: "m1",
    ...
    mockUseSendChatMessageMutation.mockReturnValue({ mutateAsync: vi.fn(), isPending: false });
  ...
  it("renders pipeline offer buttons and sends action tokens", async () => {
    ...
    mockUseSendChatMessageMutation.mockReturnValue({ mutateAsync, isPending: false });
