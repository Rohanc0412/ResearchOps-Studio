...
import { FolderKanban, FolderPlus, PanelLeftClose, PanelLeftOpen, ShieldCheck } from "lucide-react";
...
export function Sidebar() {
  ...
  return (
    <aside
      className={cx(
        "group hidden flex-col border-r border-slate-800 bg-slate-950 transition-[width] duration-200 md:flex",
        collapsed ? "w-16" : "w-64"
      )}
    >
      <div
      ...
      <nav className="flex flex-1 flex-col gap-3 px-2 py-2">
        ...
        <div className="mt-4 flex flex-col gap-1">
          {!collapsed ? (
            <div className="px-2 text-xs font-semibold uppercase text-slate-500">Account</div>
          ) : null}
          <NavLink
            to="/security"
            className={({ isActive }) =>
