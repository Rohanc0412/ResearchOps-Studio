...
class OutlineSection(BaseModel):
    ...
    section_id: str  # e.g. "intro", "methods", "conclusion"
    title: str
    goal: str
    key_points: list[str] = Field(default_factory=list)
    suggested_evidence_themes: list[str] = Field(default_factory=list)
    section_order: int
...
class OrchestratorState(BaseModel):
    ...
    # Stage 3: Selected sources (retrieval output)
    vetted_sources: list[SourceRef] = Field(default_factory=list)

    # Stage 4: Outlining
    outline: OutlineModel | None = None

    # Stage 4.5: Evidence packs (section-level gating)
    section_evidence_snippets: dict[str, list[EvidenceSnippetRef]] = Field(default_factory=dict)
    ...
    repair_edits_json: list[dict] = Field(default_factory=list)
