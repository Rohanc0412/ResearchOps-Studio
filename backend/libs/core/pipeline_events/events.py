"""
Pipeline event emission for orchestrator nodes.
...
def _state_summary(state: Any) -> dict[str, Any]:
    ...
    if outline is not None:
        sections = (
            ...
            else getattr(outline, "sections", None)
        )
    ...
    if evaluator_decision:
        summary["evaluator_decision"] = getattr(
            evaluator_decision, "value", str(evaluator_decision)
        )
...
def _stage_input_details(stage: str, state: Any) -> dict[str, Any]:
    ...
    if stage == "outline":
        details["vetted_sources_preview"] = _source_preview(
            _get_state_value(state, "vetted_sources")
        )
    if stage == "draft":
        details["outline_sections"] = _outline_preview(_get_state_value(state, "outline"))
