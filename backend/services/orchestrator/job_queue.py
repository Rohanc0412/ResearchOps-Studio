...
def enqueue_run_job_sync(*, session: Session, tenant_id: UUID, run_id: UUID, job_type: str) -> UUID:
    """Ensure a queued job exists for the run (sync version for orchestrator worker)."""
    existing = (
        session.execute(
            select(JobRow.id)
            .where(
...
enqueue_run_job = enqueue_run_job_sync


async def enqueue_run_job_async(*, session: AsyncSession, tenant_id: UUID, run_id: UUID, job_type: str) -> UUID:
    """Ensure a queued job exists for the run (async version for API service)."""
    existing = (
        (
