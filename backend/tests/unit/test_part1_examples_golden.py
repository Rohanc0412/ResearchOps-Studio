...
def test_examples_golden(repo_root: Path, evidence_store) -> None:
    ...
    valid_report = StructuredReport.model_validate(_load_json(repo_root / "tests" / "golden" / "valid_report.json"))
    claim_enforcer.enforce_report(valid_report)
    EvidenceValidator.validate_report(valid_report, evidence_store)

    valid_lit = LiteratureMap.model_validate(
        _load_json(repo_root / "tests" / "golden" / "valid_literature_map.json")
    )
    EvidenceValidator.validate_literature_map(valid_lit, evidence_store)

    valid_plan = ExperimentPlan.model_validate(
        _load_json(repo_root / "tests" / "golden" / "valid_experiment_plan.json")
    )
    EvidenceValidator.validate_experiment_plan(valid_plan, evidence_store)

    rejected_report = StructuredReport.model_validate(
        _load_json(repo_root / "tests" / "golden" / "rejected_report_missing_citations.json")
    )
